...
/**
 * [getSheet 获取excel文件内容]
 * @method getSheet
 * @param  {[type]}   ctx  [上下文]
 * @param  {Function} next [下一个处理器]
 * @return {Promise}       [获取excel内容，传输给前台使用]
 */
let fn_getSheet = async (ctx,next)=>{
...
let fn_saveSheet = async (ctx,next)=>{
...
module.exports = {
  "GET /api/sheets/:id":fn_getSheet,
  "PUT /api/sheets/:id":fn_saveSheet
}
